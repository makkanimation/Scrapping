...
$result_All = $db->getQuery("select LUID from ".$table_pre.LEAD_TEMP_DB." where matchType='not-matched' and InsertInTemp='0' order by LUID desc");
$numRows=@$db->getNumRows($result_All);
$cond=" limit $eu,$limit";
$managePage = "outlook_downloadData.php";
$result = $db->getQuery("select a.*,b.Subject from ".$table_pre.LEAD_TEMP_DB." as a INNER JOIN ".$table_pre.EMAIL_DOWNLOAD." as b ON a.ReceivedDateTime=b.ReceivedDateTime and a.Email=b.Email where matchType='not-matched' and InsertInTemp='0'  GROUP BY a.LUID order by LUID desc $cond");
...
{  
	...
	else
	{
	  ...
	  document.exportForm.action="export_unappended.php?totalRecord="+totalRecord;
