  ...
  function plainMessageHubspot($email)
	{	
  	...
  	function plainMessageHubspotUpdated($email)
	{	
		//PR($email); 
		$email = preg_replace("/<img[^>]+\>/i", "", $email); 
		$email = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $email);
		...
		$explo[1] = preg_replace("/<strong[^>]+\>/i", "<strong>", $explo[1]); 
		$newText11 = preg_split( '/(<strong>|<STRONG>)/', $explo[1]);
		foreach($newText11 as  $test1)
		{
			$telArr = array();
			if(strstr($test1,"tel:"))
			{
				$telArr = explode("tel:",$test1);
				if(strstr($telArr[1],'"'))
				{
					$telArr = explode('"',$telArr[1]);
					$test1 = "PhoneResearch:".$telArr[0];
				}
			}
